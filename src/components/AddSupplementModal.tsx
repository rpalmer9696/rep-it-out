...
const AddSupplementModal = ({ open, onClose, onSave, selectedDate }: Props) => {
  const { data: sessionData } = useSession();
  const [isOpen, setIsOpen] = useState<boolean>(open);
  const [name, setName] = useState<string>("");
  const [amount, setAmount] = useState<number>(0);
  const [errors, setErrors] = useState<string[]>([]);
  const addSupplement = api.supplement.create.useMutation();
  ...
  return (
    <div className="max-w-96 absolute top-1/2 left-1/2 flex -translate-x-1/2 -translate-y-1/2 transform flex-col rounded bg-white px-4 sm:px-8">
      ...
      <h1 className="text-center text-3xl">Add Supplement</h1>
      <div className="p-4"></div>
      {errors.map((error, id) => (
      ...
      <div className="flex flex-row justify-between">
        ...
        <button
          ...
          onClick={() => {
            try {
              setErrors([]);
              SupplementSchema.parse({ name, amount });
